...
const CharacterCardList = ({navigation}) => {
  ...
  const getFavoriteCharacters = async () => {
    const favoriteChars = favoriteCharacters;
  ...
  const loadFavoriteCharacters = chars => {
    if (favoriteCharacters.length > 0) {
      chars.forEach(character => {
        const foundCharacter = favoriteCharacters.find(
  ...
  const removeFavorite = async name => {
    const favChars = favoriteCharacters;
    const index = favChars
      .map(function (favChar) {
        return favChar.name;
      })
      .indexOf(name);
    if (index > -1) {
  ...
  return (
    <View style={CharacterCardListStyle.container}>
      {loading ? (
        <Text style={CharacterCardListStyle.loadingText}>{strings.TEXT_LOADING}</Text>
      ) : (
        <>
